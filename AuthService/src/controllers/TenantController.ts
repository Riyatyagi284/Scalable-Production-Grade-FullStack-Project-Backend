...
export class TenantController {
    ...
    async create(req: CreateTenantRequest, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const userId = req.auth?.sub; // because AuthRequest has `id`

            if (!userId) {
                throw createHttpError(400, "User ID missing from token");
            }

            // 3. Assign tenant to the user
            await this.userService.assignTenant(Number(userId), tenant.id);

            this.logger.info("Tenant has been created & assigned", {
                tenantId: tenant.id,
